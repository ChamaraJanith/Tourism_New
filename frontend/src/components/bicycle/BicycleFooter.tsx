...
import { Mail, MapPin, Phone, Bike, Trees } from "lucide-react";
import { FaFacebookF, FaInstagram, FaXTwitter, FaYoutube } from "react-icons/fa6";
import Link from "next/link";
import { fireBookingConfetti } from "@/lib/booking-confetti";
 
export const BicycleFooter = () => {
  return (
    <footer className="bg-[#080a0d] pt-40 pb-16 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-24 mb-32">
          ...
          <div>
            ...
            <ul className="space-y-6">
              {["Village Tours", "Lakeside Cycling", "Forest Adventures", "Cultural Rides", "Sunrise Expeditions"].map((item, i) => (
                <li key={i}>
                  <Link href="#" className="text-emerald-50/40 hover:text-emerald-400 transition-colors duration-500 font-light text-lg flex items-center gap-5 group">
          ...
          <div>
            <h4 className="text-white font-bold mb-12 uppercase tracking-[0.5em] text-[10px]">The Hub</h4>
            <ul className="space-y-10">
              <li className="flex items-start gap-6">
                <MapPin className="w-7 h-7 text-emerald-500 mt-1 shrink-0" />
                <span className="text-emerald-50/40 font-light text-xl leading-relaxed">Southern Adventure Hub, <br /> Galle District, Sri Lanka</span>
              </li>
              <li className="flex items-center gap-6">
                <Phone className="w-7 h-7 text-emerald-500 shrink-0" />
                <span className="text-emerald-50/40 font-light text-xl">+94 91 555 7777</span>
              </li>
              <li className="flex items-center gap-6">
                <Mail className="w-7 h-7 text-emerald-500 shrink-0" />
                <span className="text-emerald-50/40 font-light text-xl">ride@traildiscovery.lk</span>
              </li>
            </ul>
          </div>
 
          <div>
            <h4 className="text-white font-bold mb-12 uppercase tracking-[0.5em] text-[10px]">Trail Updates</h4>
            <p className="text-emerald-50/40 font-light mb-10 text-xl leading-relaxed">Join our mailing list for seasonal trail maps and adventure stories.</p>
